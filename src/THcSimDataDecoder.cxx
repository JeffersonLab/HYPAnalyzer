...
std::vector<THcSimDataDecoder*> THcSimDataDecoder::fEncoders;

THcSimDataDecoder* THcSimDataDecoder::GetEncoderByName(
    const char *enc_name)
{
  if(fEncoders.empty()) { // First generate the list of known encoders!!
    unsigned short ids = 1;
    // TDCs
    fEncoders.push_back(new THcSimTDCEncoder("caen1190",ids++,19,26));
    fEncoders.push_back(new THcSimTDCEncoder("lecroy1877",ids++,16,16));
    fEncoders.push_back(new THcSimTDCEncoder("vetroc",ids++,16,26));
    fEncoders.push_back(new THcSimTDCEncoder("f1tdc",ids++,16,31));
    // ADCs
    //fEncoders.push_back(new THcSimFADC250Encoder("fadc250",ids++));
    fEncoders.push_back(new THcSimSADCEncoder("fadc250",ids++));
    fEncoders.push_back(new THcSimADCEncoder("adc",ids++,12));
    fEncoders.push_back(new THcSimADCEncoder("lecroy1881",ids++,14));
    fEncoders.push_back(new THcSimADCEncoder("caen792",ids++,12));
    //fEncoders.push_back(new THcSimSADCEncoder("mpd",ids++));
    fEncoders.push_back(new THcSimMPDEncoder("mpd",ids++));
  }

  TString name(enc_name);
  for(std::vector<THcSimDataDecoder*>::iterator it = fEncoders.begin();
      it != fEncoders.end(); it++) {
...
THcSimDataDecoder* THcSimDataDecoder::GetEncoder(unsigned short id)
{
  for(std::vector<THcSimDataDecoder*>::iterator it = fEncoders.begin();
      it != fEncoders.end(); it++) {
    if((*it)->GetId() == id)
...
THcSimDataDecoder::THcSimDataDecoder(const char *enc_name,
    unsigned short enc_id) : fName(enc_name), fEncId(enc_id)
{
}

THcSimTDCEncoder::THcSimTDCEncoder(const char *enc_name,
    unsigned short enc_id, unsigned short bits, unsigned short edge_bit)
  : THcSimDataDecoder(enc_name,enc_id), fBits(bits), fEdgeBit(edge_bit)
{
  fBitMask = MakeBitMask(fBits);
}

THcSimADCEncoder::THcSimADCEncoder(const char *enc_name,
    unsigned short enc_id, unsigned short bits)
  : THcSimDataDecoder(enc_name,enc_id), fBits(bits)
{
  fBitMask = MakeBitMask(fBits);
}

THcSimSADCEncoder::THcSimSADCEncoder(const char *enc_name,
    unsigned short enc_id) : THcSimADCEncoder(enc_name,enc_id,12)
{
}

/*
THcSimFADC250Encoder::THcSimFADC250Encoder(const char *enc_name,
    unsigned short enc_id) : THcSimADCEncoder(enc_name,enc_id,12)
{
}
*/

unsigned int THcSimDataDecoder::MakeBitMask(unsigned short bits)
{
  unsigned int mask = 0;
...
/*
bool THcSimADCEncoder::EncodeADC(SimEncoder::adc_data data,
...
*/
/*
bool THcSimTDCEncoder::EncodeTDC(SimEncoder::tdc_data data,
    unsigned int *enc_data,unsigned short &nwords)
{
...
*/
/*
bool THcSimFADC250Encoder::EncodeFADC(SimEncoder::fadc_data data,
    unsigned int *enc_data, unsigned short &nwords)
{
  // Word 1:
...
bool THcSimADCEncoder::DecodeADC(SimEncoder::adc_data &data,
...
bool THcSimTDCEncoder::DecodeTDC(SimEncoder::tdc_data &data,
...
bool THcSimSADCEncoder::DecodeSADC(SimEncoder::sadc_data &data,
    const unsigned int *enc_data,unsigned short nwords)
{
  data.integral = 0;
...
/*
bool THcSimFADC250Encoder::DecodeFADC(//SimEncoder::fadc_data &data,
...
*/
/*
unsigned int THcSimFADC250Encoder::EncodeSingleSample(unsigned int dat)
{
...
*/
/*
void THcSimFADC250Encoder::UnpackSamples(unsigned int enc_data,
...
unsigned int THcSimDataDecoder::EncodeHeader(unsigned short type,
...
void THcSimDataDecoder::DecodeHeader(unsigned int hdr, unsigned short &type, unsigned short &ch,
    unsigned int &nwords)
{
  type = DecodeType(hdr);
...
unsigned short THcSimDataDecoder::DecodeChannel(unsigned int hdr) {
  return (hdr>>SBS_CHANNEL_FIRST_BIT)&SBS_CHANNEL_MASK;
}

unsigned short THcSimDataDecoder::DecodeType(unsigned int hdr) {
  return (hdr>>SBS_TYPE_FIRST_BIT)&SBS_TYPE_MASK;
}

unsigned short THcSimDataDecoder::DecodeNwords(unsigned int hdr) {
  return hdr&SBS_NWORDS_MASK;
}

THcSimMPDEncoder::THcSimMPDEncoder(const char *enc_name,
    unsigned short enc_id) : THcSimDataDecoder(enc_name,enc_id)
{
/*
...
/*
bool THcSimMPDEncoder::EncodeMPD(SimEncoder::mpd_data data,
    unsigned int *enc_data, unsigned short &nwords)
{
  // Word 1:
...
*/
/*
unsigned int THcSimMPDEncoder::EncodeSingleSample(unsigned int dat)
{
...
*/
/*
void THcSimMPDEncoder::UnpackSamples(unsigned int enc_data,
    unsigned int *buff, bool *overflow, bool *valid)
{
  unsigned int tmp;
  for(int k = 0; k < 2; k++) {
...
*/
/*
void THcSimMPDEncoder::EncodeMPDHeader(SimEncoder::mpd_data data,
    unsigned int *enc_data, unsigned short &nwords)
{
  enc_data[nwords++] =
...
*/
/*
void THcSimMPDEncoder::DecodeMPDHeader(const unsigned int *hdr,
...
*/
bool THcSimMPDEncoder::DecodeMPD(SimEncoder::mpd_data &data,
				 const unsigned int *enc_data,unsigned short nwords)
{
  data.samples.clear(); // Clear out any samples already in the data
