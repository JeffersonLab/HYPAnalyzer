...
//#include <THcSimFadc250Module.h>// we need not to need this
#include "TList.h"
#include "TObject.h"
...
ClassImp(THcSimDecoder) // Implements THcSimDecoder
...
//-----------------------------------------------------------------------------
THcSimDecoder::THcSimDecoder()// : fCheckedForEnabledDetectors(false), fTreeIsSet(false)
{
  // Constructor
  DefineVariables();
  fDetectors.clear();
  ...
  //cout << " Calling THcSimDecoder! "<< endl;
  ...
  fDecoderMPD = dynamic_cast<THcSimSADCEncoder*>
    (THcSimDataDecoder::GetEncoderByName("mpd"));
...
//-----------------------------------------------------------------------------
Int_t THcSimDecoder::DefineVariables( THaAnalysisObject::EMode mode )
{
  // Define global variables for the MC quantities. Extends the base
  // class method.
  
  const char* const here = "THcSimDecoder::DefineVariables";
  ...
  cout << "Read THcSimDecoder variables " << endl;
...
//-----------------------------------------------------------------------------
void THcSimDecoder::Clear( Option_t* opt )
{
...
int THcSimDecoder::LoadEvent(const UInt_t* evbuffer )
#else
int THcSimDecoder::LoadEvent(const Int_t* evbuffer )
#endif
{
...
//-----------------------------------------------------------------------------
#if ANALYZER_VERSION_CODE >= ANALYZER_VERSION(1,6,0)
Int_t THcSimDecoder::DoLoadEvent(const UInt_t* evbuffer )
#else
Int_t THcSimDecoder::DoLoadEvent(const Int_t* evbuffer )
#endif
{
  ...
  */
  ...
  static const char* const here = "THcSimDecoder::LoadEvent";
  ...
  // if(!fTreeIsSet){
  //   std::cerr << "THcSimDecoder Tree not initialized correctly - exiting" << std::endl;
  //   return HED_FATAL;
  // }
  //fTree->GetEntry(GetEvNum());
  // Cast the evbuffer pointer back to exactly the event type that is present
  // in the input file (in THcSimFile). The pointer-to-unsigned integer is
  // needed compatibility with the standard decoder.
  if(fDebug>2)std::cout << "Processing " << here << std::endl;
  
  const THcSimEvent* simEvent = reinterpret_cast<const THcSimEvent*>(buffer);
  // add a check here!!!
...
//Utilities
/*
Int_t THcSimDecoder::RetrieveDetMapParam(const char* detname, 
...
Int_t THcSimDecoder::LoadDetector( std::map<Decoder::THaSlotData*,
				   std::vector<UInt_t> > &map,
				   const std::string& detname,
				   const THcSimEvent* simev)
{
  if(fDebug>1)std::cout << "THcSimDecoder::LoadDectector(" << detname << ")" << std::endl;
  ...
  if(strcmp(detname.c_str(), "bb.ps")==0){
    ...
    for(int j = 0; j<simev->Tgmn->Earm_BBPS_dighit_nchan; j++){
      loadevt = false;
      //if(simev->Tgmn->Earm_BBPS_dighit_samp->at(j)==0)cout << "THcSimDecoder, BBPS " << simev->Tgmn->Earm_BBPS_dighit_chan->at(j);// << endl;
	...
	*/
	...
	// map is an argument passed to THcSimDecoder::LoadDetector by reference, of type std::map<Decoder::THaSlotData*, std::vector<UInt_t> >
	...
	if(!samps.empty()){
	  myev->push_back(THcSimDataDecoder::EncodeHeader(5, chan, samps.size()));
	...
	sldat = crateslot[idx(crate,slot)].get();
      ...
      myev->push_back(THcSimDataDecoder::EncodeHeader(6, chan, 1));
  ...
  if(strcmp(detname.c_str(), "bb.sh")==0){
    ...
    for(int j = 0; j<simev->Tgmn->Earm_BBSH_dighit_nchan; j++){
      loadevt = false;
      //if(simev->Tgmn->Earm_BBSH_dighit_samp->at(j)==0)cout << "THcSimDecoder, BBSH " << simev->Tgmn->Earm_BBSH_dighit_chan->at(j);// << endl;
  ...
  if(strcmp(detname.c_str(), "bb.hodo")==0){
    ...
    std::vector<UInt_t> *myev = &(map[sldat]);
    myev->push_back(THcSimDataDecoder::EncodeHeader(1, chan, 2));
    myev->push_back(0);
    */
    int ntdc = 0;
	...
	sldat = crateslot[idx(crate,slot)].get();
      ...
      if(ntdc){
	std::vector<UInt_t> *myev = &(map[sldat]);
	myev->push_back(THcSimDataDecoder::EncodeHeader(1, chan, ntdc));
	...
	sldat = crateslot[idx(crate,slot)].get();
      ...
      myev->push_back(THcSimDataDecoder::EncodeHeader(8, chan, 1));
  ...
  if(strcmp(detname.c_str(), "bb.gem")==0){
	...
	if(!samps.empty()){
	  //myev->push_back(THcSimDataDecoder::EncodeHeader(5, apvnum, samps.size()));
	  //I think I'm onto something here, but I also need to transmit strip num 
	  myev->push_back(THcSimDataDecoder::EncodeHeader(9, apvnum, samps.size()));
  ...
  if(strcmp(detname.c_str(), "sbs.hcal")==0){
	...
	if(loadevt){
	  ...
	  myev = &(map[sldat]);
	  if(!times.empty()){
	    myev->push_back(THcSimDataDecoder::EncodeHeader(4, chan, times.size()));
  ...
  if(strcmp(detname.c_str(), "sbs.active_ana")==0){
    ...
    for(int j = 0; j<simev->Tgenrp->Harm_ActAn_dighit_nchan; j++){
      loadevt = false;
      //if(simev->Tgenrp->Harm_ActAn_dighit_samp->at(j)==0)cout << "THcSimDecoder, BBSH " << simev->Tgenrp->Harm_ActAn_dighit_chan->at(j);// << endl;
  ...
  while(j < HitData_Det->nhits){
	...
	std::cout << "j = " << j << " header = " << HitData_Det->dataword->at(j) << std::endl;
      THcSimDataDecoder::DecodeHeader(HitData_Det->dataword->at(j),
    ...
    //save the header
    std::vector<UInt_t> *myev = &(map[sldat]);
    myev->push_back(THcSimDataDecoder::EncodeHeader(data_type,chan,nwords));
    if(detname.find("gem")!=std::string::npos){
...
/*
void THcSimDecoder::SetDetMapParam(const std::string detname, int cps, int spc, int fs, int fc)
{
...
void THcSimDecoder::CheckForEnabledDetectors()
{
...
/*
void THcSimDecoder::SetTree(TTree *t)
{
  if(t==0)return;
...
void THcSimDecoder::SetDetectors()
{
  //std::cout << "[THcSimDecoder::SetDetectors()]: rundate = ";
...
Int_t THcSimDecoder::AddDetector(std::string detname, TDatime date)
{
  fDetectors.push_back(detname);
  return ReadDetectorDB(detname, date);
}

Int_t THcSimDecoder::ReadDetectorDB(std::string detname, TDatime date)
{
  ...
  // FILE *file = Podd::OpenDBFile( detname.c_str(), date, "THcSimDecoder::ReadDetectorDB()", 
  // 				 "r", 2 );
...
//-----------------------------------------------------------------------------
//static inline
void THcSimDecoder::ChanToROC(const std::string& detname, Int_t h_chan,
...
int THcSimDecoder::APVnum(const std::string& detname, Int_t mod, Int_t h_chan,
			  Int_t &crate, Int_t &slot, UShort_t &chan) const
{
  chan = h_chan%128;
...
//-----------------------------------------------------------------------------
Int_t THcSimDecoder::ChanFromROC( Int_t crate, Int_t slot, Int_t chan ) const
{
