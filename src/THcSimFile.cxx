...
//   uses them to fill a THcSimEvent object. A pointer to the event
...
//-----------------------------------------------------------------------------
THcSimFile::THcSimFile(const char* filename, const char *experiment, const char* description) :
  ...
  fEvent(0), fNEntries(0), fEntry(0), fVerbose(0)
{
  ...
  */
  ...
  if( fValidExperiments.find( fExperiment ) == fValidExperiments.end() ){ //This is not a valid experiment. Default to gmn and print a warning:
    TString fWarn;
    fWarn.Form( "THcSimFile(%s, %s, %s)", filename, experiment, description );
...
/*
//-----------------------------------------------------------------------------
THcSimFile::THcSimFile(const char* filename, const char* description, std::vector<TString> det_list) :
...
*/
//-----------------------------------------------------------------------------
THcSimFile::THcSimFile(const THcSimFile &run)
...
//-----------------------------------------------------------------------------
THcSimFile& THcSimFile::operator=(const THaRunBase& rhs)
{
  if (this != &rhs) {
    THaRunBase::operator=(rhs);
    if( rhs.InheritsFrom("THcSimFile") )
      fROOTFileName = static_cast<const THcSimFile&>(rhs).fROOTFileName;
    fROOTFile = 0;
    fTree = 0;
    fEvent = 0;
    fNEntries = fEntry = 0;
...
Int_t THcSimFile::Compare( const TObject* obj ) const
{
  // Compare two THcSimFiles. They are different if either their number
  ...
  const THcSimFile* rhsr = dynamic_cast<const THcSimFile*>(rhs);
  if( !rhsr ) return 0;
...
//-----------------------------------------------------------------------------
Int_t THcSimFile::ReadDatabase()
{
  //static const char* const here = "ReadDatabase";
  
  THaRunBase::ReadDatabase();
  
  FILE* f = Podd::OpenDBFile("run", fDate, "THcSimFile::ReadDatabase", "r", 1);
  if( !f )  return -1;
  TString expt;
  DBRequest request[] = {
...
//-----------------------------------------------------------------------------
void THcSimFile::GetExperiment(const char *experiment)
{
...
//-----------------------------------------------------------------------------
Int_t THcSimFile::Open()
{
  ReadDatabase();
  // Open ROOT input file
  if(fVerbose>0)std::cout << "THcSimFile::Open(): initialize file with experiment: " << fExperiment << std::endl;
  ...
  */
  ...
  fEvent = new THcSimEvent(fTree, fExperiment);
  //cout << fDetList.size() << endl;
  //fTree->Print();
  //fEvent = new THcSimEvent(fTree, fDetList);
...
//-----------------------------------------------------------------------------
Int_t THcSimFile::Close()
{
  if(fVerbose>0)std::cout << " THcSimFile::Close(): closing file and destroy previously configured THcSimEvent " << std::endl;
...
Int_t THcSimFile::ReadEvent()
{
  ...
  if(fVerbose>1 || fEntry%1000==0 )
    std::cout << " THcSimFile::ReadEvent() -> fEntry = " 
	      << fEntry << " / " << fNEntries
	      << std::endl;
  if( fEntry >= fNEntries )
    return EOF;
...
const UInt_t *THcSimFile::GetEvBuffer() const
#else
const  Int_t *THcSimFile::GetEvBuffer() const
#endif
{
  if( !IsOpen() ) return 0;
...
//_____________________________________________________________________________
void THcSimFile::Print( Option_t* opt ) const
{
